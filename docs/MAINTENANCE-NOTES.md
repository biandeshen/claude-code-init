...
> **最新维护清单**：见 [HANDOVER-v1.6.0.md](HANDOVER-v1.6.0.md) 第十二节。
...
init.sh 和 init.ps1 的步号**已对齐**（本轮修复）：
...
**修复内容**：init.ps1 中 6 处编号偏差已修正（9→8, 10.1→9.1, 11→10, 11.1→10.1, 12→11, 13→12, 14→13）。
...
- ~~init.sh 步号 6 和 8 缺失，有两个 "# 7"~~ → ✅ 已修复（v1.6.5 本轮 init.ps1 步号对齐后已无差异）
- ~~init.sh 和 init.ps1 的 gstack 删除在不同轮次完成（R3 vs R4），步号因此不同步~~ → ✅ 已修复（两步号均已删除 gstack）
